...
- Synced with upstream `openai/codex` main (143 commits) and adapted the fork's TUI history-cell rebrand to the new split `history_cell/` modules, ported the parameterized shell-serialization tests onto upstream's reduced `ShellModelOutput` surface, and refreshed `SpawnAgentHandler`/`WaitAgentHandler` test constructions, multi-agent test wiring, and the `Status_line_reset` items to match the new rate-limit and thread-manager APIs while keeping the fork's privacy stubs, plan/feedback removals, Jailbroken-Codex branding, composer styling, and Esc-to-return Side polish intact.
